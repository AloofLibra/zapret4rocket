...
'"${Fcyan}"'8.'"${yellow}"' Переключатель между дефолтными сриптами от bol-van и кастомными стратегиями для голосовой связи. Сейчас: '"${plain}"'['"$(grep -Eq '(^NFQWS2_PORTS_UDP=|,)50000-50099(,|[[:space:]]*$)' /opt/zapret2/config && echo "Кастомные стратегии" || echo "Скрипты bol-van")"']'"${yellow}"'
